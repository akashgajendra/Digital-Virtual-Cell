...
def interpret(run_dir=None):
    """Layer 4: name gene programs via GSEA. Run after src-mode predict."""
    from src.runs import latest_run_dir as _latest_run_dir
    if run_dir is None:
    ...
    if not act_path.exists():
        raise FileNotFoundError("Run: uv run python model.py --train-src && --predict-src first")
    import numpy as np
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--interpret",
        action="store_true",
        help="Layer 4: name gene programs via GSEA (requires src-mode run)",
    )
    args = parser.parse_args()

    if not args.train and not args.predict and not args.compare_runs and not args.verify and not args.interpret:
        parser.print_help()
